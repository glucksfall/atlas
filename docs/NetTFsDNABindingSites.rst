...
1. The 1st declares the ``SOURCE`` and the 2nd declares the ``TARGET``.
   ...
   *Atlas* understand components inside brackets (e.g., ``[lacI,lacI,BS-lacI-422-402]``) as a complex, therefore, the components are internally linked.
...
3. The 5th column declares the location of the complex components:
   ...
   Valid location name is ``cytosol``.
